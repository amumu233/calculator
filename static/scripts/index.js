...
let main = {
    isEqual : false, // 上一步是否是 等号键
    isExpress : false,  // 上一步是否是 运算符
    flag: true, // 标识 是否重新输入数字
    ...
    resize(){
        ...
        const num = (!!result.tostrin)? result.toString() : result;
    ...
    // 移除Active 类
    removeActive(){
        const _act = document.querySelector('.actiove');
        ...
        switch (ope) {
            ...
            default:
                ...
                if( ! event.classList.contains('active') ){
                    _this.removeActive();
                    event.classList.add('actiove');

                    if( _this.isEqual ) {
                        _this.register = {};
                        // 点了等号之后再点运算符 , 则把 当前的结果在进行计算
                        ...
                        if( (ope == '*' || ope == '/' ) &&  ( _this.history.operator == '+' || _this.history.operator == '-')  ){
                            // 上一步是加减法,再输入乘除,则优先乘除
                            // 把当前数字和运算符 存入处理乘除的寄存器 register 中
